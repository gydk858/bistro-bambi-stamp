...
export default function StampCustomersClient() {
    ...
    const rows = users
      ...
      .map((user) => {
        ...
        return {
          ...
          current_count: card.current_count ?? 0,
          max_count: card.max_count ?? 12,
  ...
  const reload = async () => {
    ...
    try {
      ...
      setMessage('スタンプカード一覧を再読み込みしました')
    } catch (error) {
      console.error(error)
      setMessage(
        error instanceof Error
  ...
  const saveCustomer = async (customer) => {
    ...
    try {
      ...
      await syncCardImage(customer.user_id)
      await fetchCustomers(store.store_id)
  ...
  const totalStampCount = customers.reduce(
    (sum, customer) => sum + Number(customer.current_count || 0),
    0
  )

  const completedCount = customers.filter(
    (customer) => Number(customer.current_count || 0) >= Number(customer.max_count || 12)
  ).length
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        <header style={styles.header}>
          <div style={styles.brandRow}>
            ...
            <div>
              ...
              <p style={styles.subtitle}>通常スタンプ 顧客一覧</p>
              <p style={styles.headerDescription}>
                通常スタンプカードの番号、氏名、スタンプ数、メモを一覧で確認・管理します。
              </p>
          ...
          <nav style={styles.nav}>
            <Link href="/admin/stamp" style={styles.navButton}>
              カード操作
            </Link>

            <Link href="/admin/stamp/manage" style={styles.navButton}>
              イベント管理
            </Link>
        ...
        <section style={styles.summaryGrid}>
          ...
          <SummaryCard
            label="現在スタンプ合計"
            value={formatNumber(totalStampCount)}
            sub="表示店舗の合計"
          />

          <SummaryCard
            label="満了カード"
            value={`${completedCount}枚`}
            sub="最大スタンプ数に到達"
          />
        ...
        <section style={styles.panel}>
          <div style={styles.sectionHead}>
            <div>
              <h2 style={styles.sectionTitle}>スタンプカード一覧</h2>
              <p style={styles.description}>
                氏名とメモを編集できます。「カード」ボタンでスタンプカード管理画面を検索済み状態で開きます。
              </p>
            </div>
          </div>
              ...
              <table style={styles.table}>
                ...
                <tbody>
                  {filteredCustomers.map((customer) => {
                    ...
                    const isCompleted =
                      Number(customer.current_count || 0) >= Number(customer.max_count || 12)
                    ...
                    return (
                      <tr key={customer.user_id}>
                        ...
                        <td style={styles.tdCenter}>
                          <div style={isCompleted ? styles.completedStamp : styles.normalStamp}>
                            {formatNumber(customer.current_count)} / {formatNumber(customer.max_count)}
                          </div>
                        </td>
                        ...
                        <td style={styles.td}>
                          <div style={styles.actionGroup}>
                            ...
                            <Link
                              href={`/admin/stamp?user_id=${encodeURIComponent(customer.user_id)}`}
